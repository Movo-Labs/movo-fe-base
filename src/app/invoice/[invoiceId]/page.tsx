...
export default function InvoiceDetailPage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; color: string; bgColor: string }> = {
      'PREPARED': { label: 'Prepared', color: 'text-gray-700', bgColor: 'bg-gray-100' },
      'SENT': { label: 'Sent', color: 'text-blue-700', bgColor: 'bg-blue-100' },
      'PENDING_PAYMENT': { label: 'Pending', color: 'text-yellow-700', bgColor: 'bg-yellow-100' },
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium ${statusInfo.color} ${statusInfo.bgColor}`}>
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                {invoice.merchant && (
                  <div>
                    ...
                    <div className="space-y-1">
                      <p className="font-semibold text-gray-900">{invoice.merchant.businessName || 'Merchant'}</p>
                      <p className="text-sm text-gray-600">{invoice.merchant.email}</p>
                      <p className="text-xs text-gray-500 font-mono">{invoice.merchant.walletAddress}</p>
                </div>
                  </div>
                )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900">Invoice Item</h3>
              </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    <tr>
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{invoice.productName}</p>
                          {invoice.description && (
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-4">
              ...
              <div className="space-y-4">
                {/* Invoice ID */}
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Invoice ID</label>
                  <div className="flex items-center gap-2">
                    <p className="text-sm text-gray-900 font-mono truncate">{invoice.id.slice(0, 8)}...{invoice.id.slice(-4)}</p>
                    <button
                ...
                {/* Invoice Number */}
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Invoice Number</label>
                  <div className="flex items-center gap-2">
                    <p className="text-sm text-gray-900 font-mono">{invoice.invoiceNo}</p>
                    <button
                ...
                {/* Customer */}
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Customer</label>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    {invoice.usdcAmount && (
                      <p className="text-xs text-gray-500">≈ {invoice.usdcAmount} USDC</p>
                    )}
                  </div>
                </div>
                ...
                {/* Network */}
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Network</label>
                  <div className="flex items-center gap-2">
                ...
                {invoice.conversionRate && (
                  <div>
                    <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Exchange Rate</label>
                    <p className="text-sm text-gray-900">Rp {parseFloat(invoice.conversionRate).toLocaleString('id-ID')}</p>
                  </div>
                )}
                ...
                {invoice.paymentHash && (
                  <div>
                    <label className="text-xs font-medium text-gray-500 uppercase block mb-1">Tx Hash</label>
                    <div className="flex items-center gap-2">
                      <p className="text-sm text-gray-900 font-mono truncate">{invoice.paymentHash.slice(0, 6)}...{invoice.paymentHash.slice(-4)}</p>
                      <button
                ...
                <div className="mt-6 pt-6 border-t border-gray-200 space-y-3">
                  ...
                  {paymentStep && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-blue-800 text-sm font-medium">{paymentStep}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {!isConnected ? (
                    <div className="text-center py-2">
                      <p className="text-gray-600 text-sm mb-3">Connect your wallet to pay this invoice</p>
                      <ConnectButton />
                    </div>
                  ) : (
                  <button
